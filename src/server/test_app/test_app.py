...
class FlaskAppTestCase(unittest.TestCase):
    ...
    #     test_data = {
    #         'hwSetName': 'TestHWSet3-medrano_dev',
    #         'initCapacity': 20,
    #         'projectID': '001'            
    #     }
    #     response = self.app.post('/create_hardware_set', data=json.dumps(test_data), content_type='application/json')
    #     print(response.data)
        ##TODO: look into this test case, if we create two hardware sets(each with same name for same projectID) should we count it as new or flag and say its already exists???
    ...
    #         'qty': 5
    #     }
    #     response = self.app.post('/check_out', data=json.dumps(test_data), content_type='application/json')
    #     print(response.data)
    ...
    #         'qty': 10
    #     }
    #     response = self.app.post('/check_in', data=json.dumps(test_data), content_type='application/json')
    #     print(response.data)

    def test_deleteHWSet(self):
        test_data = {
            'hwSetName': 'med_dev1',
            'projectID': '001'
        }
        response = self.app.post('/delete_hardware_set', data=json.dumps(test_data), content_type='application/json')
        print(response.data)
